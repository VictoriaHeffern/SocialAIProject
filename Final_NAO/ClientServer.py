    ...
    def recognize_speech(self):
        ...
        with sd.RawInputStream(samplerate=self.samplerate, blocksize=8000, dtype=self.dtype,
            ...
            while self.listening:
                ...
                # Send heartbeat during silence
                    try:
                        if self.connected:
                            self.client_socket.send(json.dumps({"status": "listening"}).encode())
                    except:
                        pass
                    continue
    ...
    def listen_for_input(self):
        ...
        while self.listening and True:
            ...
            with self.listen_lock:
                ...
                # Check if user said a name that matches an existing profile
                # --- Detect if user self-identifies with a known profile ---
                if user_input:
                    memory_path = get_memory_database_path()
                    known_profiles = [f.replace(".txt", "") for f in os.listdir(memory_path) if f.endswith(".txt")]

                    for name in known_profiles:
                        if name.lower() in user_input.lower():

                            # If not already on this profile
                            if self.ai_model.person_name != name:
                                print(f"User self-identified as {name}. Switching profile...")

                                # Update AI model state
                                self.ai_model.face_known = True
                                self.ai_model.person_name = name
                                self.ai_model.memory_file_path = os.path.join(memory_path, f"{name}.txt")

                                # Force reload of system prompt
                                self.ai_model._prepare_system_prompt()

                                apology = f"Oh! I'm sorry for the confusion — thanks for telling me your name, {name}. Switching to your profile now."

                                self.send_speech({"response": apology})
                                print(f"NAO REPLY: {apology}")

                                time.sleep(len(apology) / charPerSec)
