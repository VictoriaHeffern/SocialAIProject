...
# function to retreive an embedding from an image
...
# function to detect single faces in an image
...
# function to init known faces from the face database
def load_embedding(relative_face_path):
    ...
    for person in os.listdir(face_loc):
        ...
        person_embeddings = [] # store ALL embeddings for this person
        ...
        avg_embedding = np.mean(person_embeddings, axis=0) # ave embeddings

        avg_embedding = avg_embedding / np.linalg.norm(avg_embedding) # normalize again after averaging 

        known_embeddings.append(avg_embedding) # store
...
# function for facial detection linked to computer Cam
def ident_in_frame(cap, embeddings, names):
    ...
    best_name_conf = -1
    detection_conf = -1
    for i in range(0, detections.shape[2]): # Loop through each face detected on screen, save largest bbox_area and face
        
        conf = detections[0, 0, i, 2] # confidence scoring
        if conf < 0.6:
            continue
        ...
        scores = [cosine(embedding, kb) for kb in embeddings] # Compare to known embeddings
...
# function for area of bounding box
