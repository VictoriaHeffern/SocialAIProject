...
# Look in the folder to find the person's profile
...
# Creates a new text file for an unknown user
...
# loads profile as dictionary with fields
...
# Parse through the response to get the chat and json messaging seperatley
...
class AIConvoModel:
    ...
    def get_initial_greeting(self):
        # If system prompt is only message, generate the first greeting
        if len(self.conversation_history) == 1:
            response = self.ask_gpt("Start the conversation by greeting the user using their profile.")
            return response if response else "Hello! Nice to meet you."
        else:
            last = self.conversation_history[-1]["content"]
            return last if last else "Hello! Nice to meet you."
    ...
    def ask_gpt(self, user_text):
        ...
        try:
            ...
            if self.memory_file_path and json_text:
                ...
                # Rename unknown user file once name is known
